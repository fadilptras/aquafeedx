import { Thermometer, Droplets, FlaskConical, Eye, Utensils, Clock, Camera } from 'lucide-react';
    ...
    <div className="space-y-6 max-w-7xl mx-auto p-2">
      ...
      <div className="fade-up">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-primary">Live Monitoring</span>
        </div>
        <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-foreground">
          AquaFeed<span className="text-primary text-4xl">X</span>
        </h1>
        <p className="text-sm text-muted-foreground">Pantau kondisi kolam dan pakan secara langsung.</p>
      </div>
      ...
      <div className="fade-up fade-up-delay-1">
        <div className="relative rounded-2xl overflow-hidden border border-white/10 shadow-2xl group">
          ...
          <div className="absolute top-4 left-4 z-20 flex items-center gap-2 bg-black/40 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/10">
            ...
            <span className="text-[10px] text-white font-bold uppercase tracking-wider">Bioflok 1 : Apa namanya?</span>
          </div>
        </div>
      </div>
            ...
            >
              ...
              <div className="z-10 flex-1">
                ...
                <div className="flex items-center gap-2">
                  <div className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md text-[9px] font-bold uppercase ${statusBg[card.status]} ${statusColors[card.status]}`}>
                    <span className={`w-1 h-1 rounded-full ${card.status === 'good' ? 'bg-success' : card.status === 'warning' ? 'bg-warning' : 'bg-destructive'} animate-pulse`} />
